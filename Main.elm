...
randomWorld : Random.Generator (World Shape)
randomWorld =
    Random.map2
        (\b1 b2 ->
            initialWorld
                |> World.add (Body.moveBy { x = -0.5, y = 0, z = 3 } b1)
                |> World.add (Body.moveBy { x = 0.5, y = 0, z = 0 } b2)
        )
        randomBox
        randomBox
    ...
    World.empty
        ...
        |> World.add
            (plane
                ...
                |> Body.rotateBy (pi / 3) { x = 0, y = 1, z = 0 }
            )
        |> World.add
            (plane
                |> Body.moveBy { x = 4, y = 0, z = 0 }
...
update msg model =
    case msg of
        ...
        Tick dt ->
            ( { model | world = World.simulate (min dt (1000 / 30)) model.world }
            , Cmd.none
            )
